...
class FlashcardBulkCreateView(LoginRequiredMixin, View):
    ...
    def get(self, request: Any, deck_pk: int) -> HttpResponse:
        return render(
            request, self.template_name, {"deck": get_object_or_404(Deck, pk=deck_pk)}
        )
    ...
    def post(self, request: Any, deck_pk: int) -> HttpResponse:
        ...
        try:
            existing_terms = set(
                Flashcard.objects.filter(deck=deck, is_active=True).values_list(
                    "term", flat=True
                )
            )
            llm_service = LLMGeneratorService()
            ...
            if created_count > 0:
                messages.success(
                    request, f"Создано {created_count} карточек по теме «{topic}»"
                )
            else:
                messages.warning(
                    request, "Не удалось создать карточки. Попробуйте другую тему."
                )
        except Exception as e:
