...
class FlashcardCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form: Any) -> HttpResponse:
        ...
        try:
            ...
            generated = llm_service.generate_example_sentence(term=term, definition='', part_of_speech='', target_language=deck.target_language)
...
class FlashcardBulkCreateView(LoginRequiredMixin, View):
    ...
    SQL: sql/queries.sql, строка 76 (проверка существующих терминов)
    SQL: INSERT INTO flashcards (...) VALUES (...) (пакетно)
    """
    ...
    def get(self, request: Any, deck_pk: int) -> HttpResponse:
        return render(request, self.template_name, {'deck': get_object_or_404(Deck, pk=deck_pk)})
    ...
    def post(self, request: Any, deck_pk: int) -> HttpResponse:
        ...
        try:
            existing_terms = set(Flashcard.objects.filter(deck=deck, is_active=True).values_list('term', flat=True))
            llm_service = LLMGeneratorService()
            cards_data = llm_service.generate_cards_by_topic(topic=topic, count=count + len(existing_terms), target_lang=deck.target_language, native_lang=deck.native_language)
            created_count = 0
            for card_data in cards_data:
                ...
                if term and translation:
                    Flashcard.objects.create(deck=deck, term=term, definition=translation, part_of_speech=str(card_data.get('part_of_speech', '')), example_sentence=str(card_data.get('example', '')), base_difficulty=3)
            ...
            if created_count > 0:
                ...
                messages.warning(request, "Все карточки по этой теме уже существуют.")
        except Exception as e:
...
class FlashcardUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request: Any, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if not (flashcard.deck.owner == request.user or request.user.is_staff):
            return redirect('cards:deck_detail', pk=flashcard.deck.pk)
        return super().dispatch(request, *args, **kwargs)

    def get_success_url(self) -> str:
        return reverse('cards:deck_detail', kwargs={'pk': self.object.deck.pk})
