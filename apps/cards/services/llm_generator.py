...
class LLMGeneratorService:
    ...
    def _generate_wrong_answers_ai(self, term: str, correct: str, count: int = 3) -> List[str]:
    ...
    def generate_cards_by_topic(
        ...
        try:
            ...
            print(f"DEBUG CLEANED JSON: {raw_content}")

            data = json.loads(raw_content)
            cards = data.get('words', [])
            
            # Если words пустой, пробуем другие ключи
            if not cards:
            ...
            print(f"DEBUG PARSED CARDS: {cards}")
            ...
            for card in cards[:count]:
                ...
                term = card.get('term') or card.get('word') or card.get('original') or ''
                translation = card.get('translation') or card.get('meaning') or card.get('native') or ''
                
                if not term or not translation:
                    print(f"DEBUG SKIPPED CARD: {card}")
                    continue
                
                result.append({
                    'term': term.strip(),
                    'translation': translation.strip(),
                    'part_of_speech': card.get('part_of_speech', card.get('pos', '')),
                    'example': card.get('example', card.get('sentence', '')),
            ...
            # Fallback: создаем тестовые карточки
            return self._generate_fallback_cards(topic, count)
    ...
    def _generate_fallback_cards(self, topic: str, count: int) -> List[Dict[str, Any]]:
        ...
        base_words = words.get(topic_lower, ['word1', 'word2', 'word3', 'word4', 'word5'])
