...
class SRSEngine:
    ...
    def reset_all_progress(user, deck: Deck) -> None:
        ...
        """
        now = timezone.now()
        UserCardProgress.objects.filter(user=user, flashcard__deck=deck).update(
            next_review_at=now,
            repetition_number=0,
            easiness_factor=2.5,
    ...
    def initialize_deck_progress(user, deck: Deck) -> int:
        ...
        for card in cards:
            ...
            if created:
                created_count += 1
                print(
                    f"DEBUG INIT: created progress for {card.term}, next_review = {now}"
                )
            else:
                print(
                    f"DEBUG INIT: progress exists for {card.term}, next_review = {progress.next_review_at}"
                )
    ...
    def get_due_cards(user, deck: Deck) -> QuerySet[Flashcard]:
        ...
        for p in all_progress:
            print(
                f"  - Card: {p.flashcard.term}, next_review: {p.next_review_at}, EF: {p.easiness_factor}"
            )
        ...
        return Flashcard.objects.filter(pk__in=due_progress.values("flashcard_id"))

    @staticmethod
    def process_review(progress: UserCardProgress, quality: int) -> UserCardProgress:
        """
        Обработка ответа пользователя (quality: 0-5).
        quality >= 4 — правильный ответ
        quality < 4 — неправильный
        """
        ...
        if quality >= 4:
            ...
            # Неправильный ответ — вернуть карточку сегодня
            progress.repetition_number = 0
            progress.inter_repetition_interval = 0
            progress.consecutive_correct = 0
            progress.total_errors += 1
            progress.next_review_at = timezone.now()  # ← СЕГОДНЯ
    ...
    def _update_deck_progress(user, deck: Deck) -> None:
        ...
        """
        progress, _ = DeckProgress.objects.get_or_create(user=user, deck=deck)

        all_progress = UserCardProgress.objects.filter(
            user=user, flashcard__deck=deck, flashcard__is_active=True
        )

        # Новый критерий: EF >= 2.0 И хотя бы 1 правильный ответ
        progress.cards_mastered = all_progress.filter(
            easiness_factor__gte=1.5, consecutive_correct__gte=1
        ).count()

        # В процессе: те, у кого есть расписание, но они не освоены
        progress.cards_learning = (
            all_progress.filter(next_review_at__isnull=False)
            .exclude(easiness_factor__gte=2.0, consecutive_correct__gte=1)
            .count()
        )

        # Сложные: EF < 1.7 или ни одного правильного ответа
        struggling_filter = Q(easiness_factor__lt=1.7) | Q(consecutive_correct=0)
        progress.cards_struggling = (
            all_progress.filter(struggling_filter).distinct().count()
        )
